        ...
        <div id="dept-detail-panel" class="card" style="display:none;margin-top:4px">
          ...
          <div class="card-body" id="dept-detail-body" style="padding:0">
            ...
            <div id="detail-employees" class="dept-detail-sub" style="padding:16px;overflow-x:auto">
              ...
              <div id="detail-emp-pagination"></div>
            </div>
            <!-- Contracts sub-panel -->
            <div id="detail-contracts" class="dept-detail-sub" style="display:none;padding:16px;overflow-x:auto">
              ...
              <div id="detail-contract-pagination"></div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="card">
          <div class="card-header" style="background:linear-gradient(135deg,#F5F3FF,#EDE9FE)">
            ...
            <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
              <select id="emp-dept-filter" class="form-input" style="font-size:12px;padding:5px 10px;width:auto" onchange="Paginator.reset('dept-all-emp');deptRenderAllEmployees()">
                <option value="">All Departments</option>
              </select>
              <input type="text" id="emp-search" placeholder="Search name, email…" style="font-size:12px;padding:5px 12px;border:1px solid #D1D5DB;border-radius:8px;width:200px" oninput="Paginator.reset('dept-all-emp');deptRenderAllEmployees()">
      ...
      <div id="tab-contracts" class="tab-content">
        <div class="card">
          <div class="card-header" style="background:linear-gradient(135deg,#EFF6FF,#DBEAFE)">
            ...
            <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
              <select id="contract-dept-filter" class="form-input" style="font-size:12px;padding:5px 10px;width:auto" onchange="Paginator.reset('dept-all-contract');deptRenderAllContracts()">
                <option value="">All Departments</option>
              </select>
              <select id="contract-expiry-filter" class="form-input" style="font-size:12px;padding:5px 10px;width:auto" onchange="Paginator.reset('dept-all-contract');deptRenderAllContracts()">
          ...
          <div class="card-body" style="overflow-x:auto">
            ...
            <div id="all-contract-pagination"></div>
          </div>
        </div>
      </div>
        ...
        <div class="card">
          <div class="card-header" style="background:linear-gradient(135deg,#FFFBEB,#FEF3C7)">
            ...
            <div style="display:flex;gap:8px;align-items:center">
              <select id="pos-dept-filter" class="form-input" style="font-size:12px;padding:5px 10px;width:auto" onchange="Paginator.reset('dept-pos');deptRenderPositions()">
                <option value="">All Departments</option>
              </select>
            </div>
          </div>
...
// ── Paginators ──
Paginator.create('dept-detail-emp',     { perPage:10, onPageChange: function(){ deptRenderDetailEmp(); }});
Paginator.create('dept-detail-contract',{ perPage:10, onPageChange: function(){ deptRenderDetailCon(); }});
Paginator.create('dept-all-emp',        { perPage:10, onPageChange: function(){ deptRenderAllEmployees(); }});
Paginator.create('dept-all-contract',   { perPage:10, onPageChange: function(){ deptRenderAllContracts(); }});
Paginator.create('dept-pos',            { perPage:10, onPageChange: function(){ deptRenderPositions(); }});
...
async function deptOpenDetail(deptName) {
  ...
  try {
    ...
    // Render employees
    window._deptDetailEmp = res.employees;
    Paginator.reset('dept-detail-emp');
    deptRenderDetailEmp();

    // Render contracts
    window._deptDetailCon = res.contracts;
    Paginator.reset('dept-detail-contract');
    deptRenderDetailCon();
...
function deptRenderDetailEmp() {
  var empTbody = document.getElementById('detail-emp-tbody');
  var data = window._deptDetailEmp || [];
  if (!data.length) {
    ...
    document.getElementById('detail-emp-pagination').innerHTML = '';
    return;
  }
  var allRows = data.map(function(e) { return '<tr>' +
    ...
    '</tr>'; });
  var paged = Paginator.paginate('dept-detail-emp', allRows);
  empTbody.innerHTML = paged.join('');
  Paginator.renderControls('dept-detail-emp', 'detail-emp-pagination');
}

function deptRenderDetailCon() {
  var conTbody = document.getElementById('detail-contract-tbody');
  var data = window._deptDetailCon || [];
  if (!data.length) {
    ...
    document.getElementById('detail-contract-pagination').innerHTML = '';
    return;
  }
  var allRows = data.map(function(c) { return '<tr>' +
    ...
    '</tr>'; });
  var paged = Paginator.paginate('dept-detail-contract', allRows);
  conTbody.innerHTML = paged.join('');
  Paginator.renderControls('dept-detail-contract', 'detail-contract-pagination');
}
...
function deptRenderAllEmployees() {
  ...
  if (!list.length) { tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:30px;color:#9CA3AF">No employees found</td></tr>'; document.getElementById('all-emp-pagination').innerHTML = ''; return; }

  var allRows = list.map(e => {
  ...
  var paged = Paginator.paginate('dept-all-emp', allRows);
  tbody.innerHTML = paged.join('');
  Paginator.renderControls('dept-all-emp', 'all-emp-pagination');
}
...
function deptRenderAllContracts() {
  ...
  if (!list.length) { tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:30px;color:#9CA3AF">No contracts found</td></tr>'; document.getElementById('all-contract-pagination').innerHTML = ''; return; }

  var allRows = list.map(c => `<tr>
  ...
  var paged = Paginator.paginate('dept-all-contract', allRows);
  tbody.innerHTML = paged.join('');
  Paginator.renderControls('dept-all-contract', 'all-contract-pagination');
}
...
function deptRenderPositions() {
  ...
  if (!list.length) { tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:30px;color:#9CA3AF">No vacant positions</td></tr>'; document.getElementById('pos-pagination').innerHTML = ''; return; }

  var allRows = list.map(p => {
  ...
  var paged = Paginator.paginate('dept-pos', allRows);
  tbody.innerHTML = paged.join('');
  Paginator.renderControls('dept-pos', 'pos-pagination');
}
