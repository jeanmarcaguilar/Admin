...
function renderRooms() {
  ...
  grid.innerHTML = rooms.map(room => {
    ...
    if (isMaint) {
      ...
      // Important override button - only for Level 3+ users
...
function renderRoomLogs() {
  ...
  tbody.innerHTML = roomLogsData.map((log, idx) => {
...
function exportRoomLogs(format) {
  ...
  const tableData = roomLogsData.map(l => {
    ...
    return [
      '#' + l.log_id,
      l.facility_name || '—',
      levelMap[l.room_level] || '—',
      l.event_title || '—',
      (l.reservation_type || '—').charAt(0).toUpperCase() + (l.reservation_type || '—').slice(1),
      l.start_datetime ? new Date(l.start_datetime).toLocaleString('en-US', { month:'short', day:'numeric', year:'numeric', hour:'2-digit', minute:'2-digit' }) : '—',
      dur,
      l.department || '—',
      l.reserved_by_name || '—',
  ...
  doc.autoTable({
    startY: 28,
    head: [['Log#','Facility','Level','Event','Type','Date & Time','Duration','Dept','By','Status']],
    body: tableData,
