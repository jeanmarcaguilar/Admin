...
switch ($action) {
    ...
    case 'upload_document':
        ...
        if (!in_array($fileExt, $allowedTypes)) {
            jsonResponse(['success' => false, 'message' => 'Invalid file type. Allowed: ' . implode(', ', $allowedTypes)], 400);
        }
        
        // Validate file size (max 10MB)
        if ($file['size'] > 10 * 1024 * 1024) {
            jsonResponse(['success' => false, 'message' => 'File too large. Maximum size is 10MB'], 400);
        }
        ...
        if (!move_uploaded_file($file['tmp_name'], $targetPath)) {
            // If move_uploaded_file fails, try copy+unlink for testing
        ...
        $validTypes = ['memo', 'contract', 'report', 'policy', 'form', 'certificate', 'invoice', 'receipt', 'letter', 'other'];
        if (!in_array($documentType, $validTypes)) {
            $documentType = 'other'; // Default to 'other' if invalid
        }
        
        $confidentiality = $_POST['confidentiality'] ?? 'internal';
        
        // Map confidentiality to valid enum values
        $validConfidentiality = ['public', 'internal', 'confidential', 'restricted'];
        if (!in_array($confidentiality, $validConfidentiality)) {
            $confidentiality = 'internal'; // Default to 'internal' if invalid
        }
        
        $documentDate = $_POST['document_date'] ?? date('Y-m-d');
        $designatedEmployee = $_POST['designated_employee'] ?? null;
        ...
        $stmt = getDocDB()->prepare("INSERT INTO documents 
            (document_code, title, document_type, description, file_path, file_name,
             file_size, file_type, uploaded_by, department, confidentiality, status,
             folder_name)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
        ...
        $result = $stmt->execute([
            ...
            $fileName, 
            $file['size'], 
            mime_content_type($uploadDir . $fileName) ?: 'application/octet-stream',
            $_SESSION['user']['user_id'] ?? 1,
            $department,
            $confidentiality,
            'active',
            $department // Use department as folder_name for organization
        ]);
        
        if (!$result) {
            // Delete uploaded file if database insert fails
            unlink($uploadDir . $fileName);
            jsonResponse(['success' => false, 'message' => 'Failed to save document record'], 500);
        }
        
        logAudit('documents', 'UPLOAD_DOCUMENT', 'documents', null, null, [
            'document_code' => $code, 
            'title' => $title,
            'file_name' => $fileName
        ]);
        
        jsonResponse(['success' => true, 'document_code' => $code, 'message' => 'Document uploaded successfully']);
